...
export default function EditPreparationPage() {
  ...
  const preparationId = params.preparationId as string;
  ...
  useEffect(() => {
    fetch(`/api/professional/preparations/${preparationId}`)
      .then((r) => r.json())
      .then((data: { preparation: Preparation }) => {
        const p = data.preparation;
        setName(p.name);
        setCategory(p.category);
        setDescription(p.description ?? "");
        setPreparationMethod(p.preparationMethod ?? "");
        setIngredients(
          p.ingredients.length > 0
            ? p.ingredients.map((ing) => ({ name: ing.name, unit: ing.unit as Unit }))
            : [{ name: "", unit: "g" }]
        );
      })
      .catch(() => setError("Falha ao carregar preparação"))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/professional/preparations/${preparationId}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Falha ao atualizar preparação");
      }

      router.push("/professional/preparations");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao atualizar preparação");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Falha ao excluir preparação");
      router.push("/professional/preparations");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao excluir preparação");
      setDeleting(false);
    }
  }
    ...
    return (
      <div className="p-4 md:p-8 max-w-[640px]">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-7 w-64 bg-[#F3F4F6] rounded" />
          <div className="h-48 bg-[#F3F4F6] rounded-xl" />
        </div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="pt-2 border-t border-[#F3F4F6]">
          <button
            ...
            disabled={deleting}
            className="w-full h-10 flex items-center justify-center gap-2 text-[13px] font-semibold text-[#DC2626] hover:bg-[#FEF2F2] rounded-[8px] transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Trash2 size={14} strokeWidth={2} />
            {deleting ? "Excluindo…" : "Excluir preparação"}
          </button>
        </div>
