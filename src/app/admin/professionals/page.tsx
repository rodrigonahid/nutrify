    ...
    <div className="p-4 md:p-8 max-w-[900px]">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-[#6B7280]">
            {professionalsList.length === 0
              ...
              : `${professionalsList.length} profissional${professionalsList.length !== 1 ? "(is)" : ""}`}
          </p>
        </div>
        <Link
      ...
      {professionalsList.length === 0 ? (
        <div className="bg-white border border-[#E5E7EB] rounded-xl flex flex-col items-center justify-center py-14 px-6 text-center">
          ...
          <p className="text-[15px] font-semibold text-[#374151] mb-1">Nenhum profissional ainda</p>
          <p className="text-[13px] text-[#9CA3AF] mb-4">Crie a primeira conta de nutricionista.</p>
          <Link
            href="/admin/professionals/create"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                    ...
                    {prof.professionalLicense && (
                      <p className="text-[12px] text-[#9CA3AF]">CRN: {prof.professionalLicense}</p>
                    )}
                  </div>
                </div>
                <div className="shrink-0 text-right">
                  <p className="text-[13px] font-semibold text-[#374151]">
                    {prof.patientCount} paciente{prof.patientCount !== 1 ? "s" : ""}
                  </p>
                  <p className="text-[11px] text-[#9CA3AF] mt-0.5">
                    {new Date(prof.userCreatedAt).toLocaleDateString("pt-BR", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                </div>
              </div>
            ))}
